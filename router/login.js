...
function setLogin(user, req, res) {
  var str = "SELECT id, fullname, userurl, passwd FROM users WHERE email = ?";
...
app.post('/auth', async (req, res) => {
  if (req.session.login) res.redirect('/'); // if user already login then skiped and go /
  else {
    // delay 2 second
    await sleep(2000);
    req.session.mid = -1;
    ...
    if (emailRegex.test(user)) {
      ...
      con.query(str, [email], (err, results) => {
        if (err) { console.error('Error executing query:', err); return; }
        ...
        // check user click next or create new account
          ...
          if (results.length == 0) {
            ...
            req.session.otp = sendOtp(email); // sent otp to provided email id
...
app.post('/auth1', async (req, res) => {
  if (req.session.login) { res.redirect('/'); return; } // if user already login then skiped and go '/'
    ...
    if (req.body.otp == req.session.otp) {
      ...
      if (req.session.mid === 0) {
        ...
        con.query(str, [req.session.email], (err, results) => {
          ...
          // insert user in user_profile table
          const sq = `INSERT INTO user_profile (id) VALUES(?)`;
          con.query(sq, [userId], (err) => {
            if (err) { return res.status(500).json({ error: 'Error creating table', details: err }); }
            // create some table to manage all task
            const sql1 = `CREATE TABLE IF NOT EXISTS \`${userId}_cond\` (id INT PRIMARY KEY)`;
            // store connected user list
            con.query(sql1, (err) => {
              if (err) { console.error('Error executing query:', err); return; }
            });

            const sql2 = `CREATE TABLE IF NOT EXISTS \`${userId}_cong\` (id INT PRIMARY KEY)`;
            // store conecting user list
            con.query(sql2, (err) => {
              if (err) { console.error('Error executing query:', err); return; }
            });

            const sql3 = `CREATE TABLE IF NOT EXISTS \`${userId}_save\` (id INT, postid INT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`;
            // store user saved post
            con.query(sql3, (err) => {
              if (err) { console.error('Error executing query:', err); return; }
            });

            const sql4 = `CREATE TABLE IF NOT EXISTS \`${userId}_requested\` (id INT PRIMARY KEY)`;
            // store reqused user id
            con.query(sql4, (err) => {
              if (err) { console.error('Error executing query:', err); return; }
            });

            const sql5 = `CREATE TABLE IF NOT EXISTS \`${userId}_message\` (namemes varchar(100), dp varchar(255), userid int, signalmes TINYINT, address varchar(100), countmes INT DEFAULT 0)`;
            // store message list details
            con.query(sql5, (err) => {
              if (err) { console.error('Error executing query:', err); return; }
            });

            const sql6 = `CREATE TABLE IF NOT EXISTS \`${userId}_notification\` (namemes varchar(100), dp varchar(255), userid int, signalmes TINYINT, address varchar(100), countmes INT DEFAULT 0, seenmes INT DEFAULT 0)`;
            // store Notification list details
            con.query(sql6, (err) => {
              if (err) { console.error('Error executing query:', err); return; }
            });
          });
          ...
          // create content post link and data storing table for user
          var columns = `id INT AUTO_INCREMENT PRIMARY KEY, filename VARCHAR(255), fullpath VARCHAR(500), title VARCHAR(1000), usignal INT, privacy int default 0, strict INT default 0, shar int default 0, comet int default 0, view1 INT default 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`;
          var sql = `CREATE TABLE IF NOT EXISTS \`${username1 + '_post'}\` (${columns})`;
          con.query(sql, (err) => {
            if (err) { console.error('Error executing query:', err); return; }
          });
        });
...
app.post('/auth3', async (req, res) => {
  ...
  // user click with otp login
    ...
    req.session.otp = await sendOtp(req.session.email); // send otp to user email
